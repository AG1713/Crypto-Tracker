...
fun LineChart(
    ...
    val visibleDataPoints = remember(dataPoints, visibleDataPointsIndices) {
        // Remember block will be recomputed everytime the keys change
    ...
    val maxYValue = remember (visibleDataPoints){
        ...
        // If the left-hand side is null, it returns the right-hand side instead.
    }
    ...
    Canvas(
        ...
        val minLabelSpacingYPx = style.minYLabelSpacingInDp.toPx()
        val verticalPaddingPx = style.verticalPadding.toPx()
        val horizontalPaddingPx = style.horizontalPadding.toPx()
        val xAxisLabelSpacingPx = style.xAxisLabelSpacing.toPx()

        // Text measurer is needed to measure some text (get the bounds of teh text)
        ...
        // Now Arranging the text based on the width of the x label with longest width,
        // so that all x labels would have same width
        ...
        val viewPortBottomY = viewPortTopY + viewPortHeightPx // Recall, canvas starts from top left corner
...
//        )
        ...
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (selectedDataPointIndex == index) {
                ...
                // Checking if selected text is visible
