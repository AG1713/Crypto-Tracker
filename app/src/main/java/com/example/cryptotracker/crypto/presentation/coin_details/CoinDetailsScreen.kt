...
import com.example.cryptotracker.crypto.presentation.toDisplayableNumber
...
fun CoinDetailsScreen(
    ...
    if (state.isLoading) {
        ...
        Column (
            modifier = modifier
                .fillMaxSize()
                .verticalScroll(rememberScrollState()) // If screen gets too large, we can enable scrolling
                .padding(16.dp),
            ...
            // FlowRow
            // It is just like normal row, but excess content is put on the next line,
            // which will be useful when managing between portrait and landscape mode
            ...
            FlowRow (
                ...
                val contentColor = if (isPositive) {
                     if (isDarkTheme) Color.Green else greenBackground
                }
                else MaterialTheme.colorScheme.error

                InfoCard(
                    title = stringResource(id = R.string.change_last24hr),
            ...
            AnimatedVisibility(
                ...
                val amountVisibleDataPoints = if (labelWidth > 0) {
                    ((totalChartWidth - 2.5*labelWidth)/labelWidth).toInt()
                } else 0
                val startIndex = (coin.coinPriceHistory.lastIndex - amountVisibleDataPoints).coerceAtLeast(0)
